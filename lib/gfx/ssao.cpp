...
@fragment fn fs_main(in: VertexOutput) -> @location(0) vec4f {
    ...
    let screen_r = clamp(params.radius / dist, 0.002, 0.05);
    var occ = 0.0;
    for (var i = 0u; i < 8u; i++) {
        let tap_uv = in.uv + kTaps[i] * screen_r;
        let tap = view_pos(tap_uv);
        let diff = dist - length(tap); // positive: tap is closer to the camera
        let range_fade = 1.0 - smoothstep(params.radius * 0.5, params.radius * 2.0, diff);
        occ += step(params.radius * 0.02, diff) * range_fade;
    }
    let ao = 1.0 - params.intensity * (occ / 8.0);
...
wgpu::BindGroupLayout make_ao_layout(bool multisampled, const char* label) {
...
void encode(const wgpu::CommandEncoder& cmd, const wgpu::TextureView& depthView, const wgpu::TextureView& colorView,
  ...
  // 2) Multiply the AO term onto the scene color.
  {
    ...
    const std::array entries{
        wgpu::BindGroupEntry{.binding = 0, .textureView = g_aoTexture.view},
